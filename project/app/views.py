...
def afficher_login(request):
    ...
    if request.method == 'POST':
        ...
        account = Account.objects.filter(account_email=data["email"]).first()
        if account is None or not check_password(data["password"], account.account_password):
...
def afficher_register(request):
    ...
    if (request.method=="POST"):
        ...
        if User.objects.filter(account__account_email=data["email"]).exists():
